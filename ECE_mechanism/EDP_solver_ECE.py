...
def Equilibrium_Matrix_ECE(Nx, n, k_p, k_m):
...
def Matrix_constant_ECE(Nx, Dt, n, k_p, k_m, DM):
    M_new_constant = Identity_troncated(Nx, n) - 0.5*DM*Laplacian_Matrix(Nx, n) - 0.5*Dt*Equilibrium_Matrix_ECE(Nx, n, k_p, k_m)
    M_old_constant = Identity_troncated(Nx, n) + 0.5*DM*Laplacian_Matrix(Nx, n) + 0.5*Dt*Equilibrium_Matrix_ECE(Nx, n, k_p, k_m)  
...
def Matrix_ECE_boundaries_red(M_new_constant, t, E, Lambda, Nx, F_norm, cst_syst): 
    ...
    M_new[2*Nx, 2*Nx]   = + 1 + Lambda*k_red_2(t, E, cst_syst, F_norm)
    M_new[2*Nx, 2*Nx+1] = - 1
    M_new[2*Nx, 3*Nx]   = - Lambda*k_ox_2(t, E, cst_syst, F_norm)
...
def k_red_1(t, E, cst_syst, F_norm):                                        ## definition de k_red_eq_a-b
    s = np.exp(-cst_syst[3]*cst_syst[2]*F_norm*(E(t) - cst_syst[0]))      
    return(s)  

def k_ox_1(t, E, cst_syst, F_norm):                                        ## definition de k_ox_eq_a-b
    s = np.exp((1-cst_syst[3])*cst_syst[2]*F_norm*(E(t) - cst_syst[0]))       
    return(s)  

def k_red_2(t, E, cst_syst, F_norm):                                        ## definition de k_red_eq_c-d
    s = np.exp(-cst_syst[3]*cst_syst[2]*F_norm*(E(t) - cst_syst[9]))      
    return(s)  

def k_ox_2(t, E, cst_syst, F_norm):                                        ## definition de k_ox_eq_c-d
    s = np.exp((1-cst_syst[3])*cst_syst[2]*F_norm*(E(t) - cst_syst[9]))       
    return(s)  

# calculating the right hand side term for the E mecanism
def RHS_ECE(M_old, C_old, cst_conc, Nx):
